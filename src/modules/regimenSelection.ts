...
function nextLineOfTherapyOrdinal(ctx: ModuleRunContext): number {
  const entries = [...ctx.canonicalInputs.values()].filter((o) => o.canonicalObjectType === "TreatmentHistoryEntry");
  const ordinals = entries.map((e) => (e.payload as { lineOfTherapyOrdinal: number }).lineOfTherapyOrdinal);
    ...
    for (const therapy of match.therapies) {
      ...
      for (const rule of rulesForTherapy) {
        ...
        if (rule.claim.contraindicationRule === "medication-interaction") {
          ...
          // Fixture convention: a lab-gate trigger fires when a lab result
          // with the matching test code carries the sentinel low-value flag.
